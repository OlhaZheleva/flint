...
function randonScreen() {
  ...
  const mobileImageEl = nextScreenEl.querySelector(`.${NEXT_SCREEN_CLASS}_mobile`);
  
  if (!pcImageEl.src && !mobileImageEl.src) {
    imagesCache[NEXT_SCREEN_CLASS].desktop.src = imagesSrc[NEXT_SCREEN_CLASS].desktop;
    imagesCache[NEXT_SCREEN_CLASS].mobile.src = imagesSrc[NEXT_SCREEN_CLASS].mobile;
...
function initAnimation (currentScreenIndex) {
  ...
  const gifCollections = currentScreen.querySelectorAll(".primary__animation-items");
